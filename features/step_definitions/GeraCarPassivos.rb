...
Quando("preencher os dados padroes") do
    PreencheCar.AbaProriedade(nomeCAR = "GW Sitio " + Faker::Name.first_name + " Passivos ",  muni = "GUARULHOS")
...
E("criar todos os desenhos") do
    Desenha.InserePoligono(tipo = "Propriedade", coordenadas = [565,354,625,343,623,380,564,359])
    ...
    Desenha.InserePoligono(tipo = 'NascenteVereda',    coordenadas = [575,210,577,184,604,201,580,212])
    ...
    #Desenha.InserePoligono(tipo = 'OutrosCorpos',  coordenadas = [])
    ...
    Desenha.InserePoligono(tipo = 'UsoConsolidadoApp', coordenadas = [357,142,975,66,973,92,356,145])
    Desenha.InserePoligono(tipo = 'UsoConsolidadoOutrasApp', coordenadas = [622,243,651,211,675,257,624,245])
...
Então("o sistema criará o CAR com Passivos") do
    PreencheCar.AbaFinalizar(possuiAreas = "")
    @numCar = find(:id, "ctl00_conteudo_lblCAR").text
    puts("CAR com Passivos criado com sucesso no link " + link = (current_url) + " e seu número é: " +  @numCar)
